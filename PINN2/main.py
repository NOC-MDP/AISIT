...
def parse_args():
    ...
    grp_train.add_argument(
        ...
        type=float,
        default=0.15,
        help="Fraction of the most recent years reserved as a final, untouched "
        "test set. Carved out before CV and before final training — never "
        "seen by any model until evaluation.",
    )
...
def main():
    ...
    print(f"  Training Data Holdout   : {args.holdout_frac}")
    print(f"\n  Physics loss weights:")
    ...
    print(f"\n── 2b. Carving out holdout test set ({args.holdout_frac:.0%}) ──")
    cutoff = np.percentile(years, 100 * (1 - args.holdout_frac))
    ...
    print(f"  Train+CV pool : {train_mask.sum():,} obs (years <= {cutoff:.0f})")
    print(f"  Held-out test : {test_mask.sum():,} obs (years >  {cutoff:.0f})")
    print("  This test set is not touched again until Step 5b.\n")
    ...
    y_test_pred_mn, y_test_pred_sd = predict_with_uncertainty(
        model_final, X_test_sc, n_samples=args.mc_samples, device=device,
    )
    ...
    print(f"  Held-out RMSE : {test_rmse:.4f}")
    print(f"  Held-out R2   : {test_r2:.3f}")
    print(f"  Compare to in-sample R2 above ({overall_r2:.3f}) — a large gap "
            f"flags overfitting.")
    ...
    test_pred_path = os.path.join(args.output_dir, "predictions_heldout_test.csv")
