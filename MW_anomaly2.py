...
def _():
    ...
    return Baltic, MassBal, Pacific, cfg, log_status, transects


@app.cell
def _(Baltic, MassBal, Pacific, cfg, log_status):
...
def _(cfg, log_status):
    ...
    return (
        mw_anomaly,
        mw_anomaly_uncertainty,
        sim_anomaly,
        sim_anomaly_uncertainty,
    )
...
@app.cell
def _(R, cfg, log_status, transects):
    ...
    for t_name, transect in transects.items():    
        ...
        cell_flux_m3s = v_normal * f_met_trans * dz_trans * da_ds

        # Sum over depth and the segments along the transect line
        total_flux_m3s = cell_flux_m3s.sum(dim=["Z", "segment"])

        # Convert from cubic meters per second to milliSverdrups (1 mSv = 1000 m3/s)
        flux_mSv = total_flux_m3s / 1000.0

        # Clean up attributes
        flux_mSv.name = "meteoric_flux_labrador"
        flux_mSv.attrs["units"] = "mSv"
        flux_mSv.attrs["description"] = (
        ...
        flux_mSv.plot(linewidth=1.5, color=cmocean.cm.balance(0.2))
        ...
        plt.ylabel("Flux ($\text{km}^3/\text{year}$)", fontsize=11)
